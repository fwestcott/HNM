...
results_df <- as.tibble(results$Condition, rownames = "Condition")
results_df <- separate(results_df, Condition, into = c("group1", "group2"), sep = "-", convert = TRUE)
results_df$p_adj_signif <- if_else(results_df$`p adj` >= 0.05, "ns", if_else(results_df$`p adj` >= 0.01, "*", if_else(results_df$`p adj` >= 0.001, "**", "***")))
...
axis.title.y = element_text(color = "grey20", size = 18)) +
  stat_pvalue_manual(pairwise.test, y.position = 900, step.increase = 0.1)
...
axis.title.x = element_text(color = "grey20", size = 18),
axis.title.y = element_text(color = "grey20", size = 18))+
  stat_pvalue_manual(pairwise.test, y.position = 35, step.increase = 0.1)
...
axis.title.x = element_text(color = "grey20", size = 18),
axis.title.y = element_text(color = "grey20", size = 18)) +
  stat_pvalue_manual(pairwise.test, y.position = 70, step.increase = 0.1)
...
axis.title.x = element_text(color = "grey20", size = 18),
axis.title.y = element_text(color = "grey20", size = 18)) +
  stat_pvalue_manual(pairwise.test, y.position = 10.5, step.increase = 0.1)
